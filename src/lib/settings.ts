...
// ── Interface preferences (font / density / ambient) ──
...
export async function getDensityPref(): Promise<boolean> {
  return (await getSetting(DENSITY_KEY)) === "condensed";
}
export async function setDensityPref(condensed: boolean) {
  await setSetting(DENSITY_KEY, condensed ? "condensed" : "comfortable");
}
export function applyDensity(condensed: boolean) {
  if (condensed) document.documentElement.dataset.density = "condensed";
